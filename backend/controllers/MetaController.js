...
metaRouter.get("/patch/role/:patch/:role", (request, response) => {
	...
	Meta.find({ patch: patch })
		.then((result) => {
			switch (role) {
				case "TOP":
					championData = result[0].champions.find(
						(championList) => championList.role === ROLES.TOP
					)
					response.json(championData)
					break
				case "JG":
					championData = result[0].champions.find(
						(championList) => championList.role === ROLES.JG
					)
					response.json(championData)
					break
				case "MID":
					championData = result[0].champions.find(
						(championList) => championList.role === ROLES.MID
					)
					response.json(championData)
					break
				case "ADC":
					championData = result[0].champions.find(
						(championList) => championList.role === ROLES.ADC
					)
					response.json(championData)
					break
				case "SUPP":
					championData = result[0].champions.find(
						(championList) => championList.role === ROLES.SUPP
					)
					response.json(championData)
					break
				default:
