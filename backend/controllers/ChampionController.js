...
championRouter.get('/:id',(request,response) => {
    ...
    Champion.findById(id).then(result => {
        response.json(result)
    }).catch(error => {
        console.log(error)
    })
...
championRouter.delete('/:id',(request,response)=>{
    let {id} = request.params
    Champion.findByIdAndDelete(id).then(result => {
        Favorito.remove({id_champion: id}).then(result => {
...
championRouter.get('/name/:name', (request,response)=>{
    ...
    Champion.find({name: name}).then(result =>{
        response.json(result)
    }).catch(e =>{
        console.log(e)
    })
